...
def project_observation_covariance(covariance: Tensor, basis: Tensor) -> Tensor:
    """Project diagonal or full CineMA covariance into the observation basis."""
...
class PerSequencePostHocSDERNN(nn.Module):
    ...
    and 22 of the SDE-RNN paper. Diffusion is fixed to G=I and Q is estimated
    from sequence innovations rather than learned through a variance loss.
    """
    ...
    def propagate_analytical(
        ...
        *,
        ...
        covariance = torch.eye(self.hidden_dim, device=hidden.device, dtype=hidden.dtype) * float(init_covariance)
        process_covariance = process_covariance.to(covariance)
        means: list[Tensor] = []
        ...
        for index in range(len(times)):
            if index > 0:
                ...
                for step in range(self.integration_steps):
                    ...
                    drift_jacobian = self._drift_jacobian(hidden, time).to(covariance)
                    covariance = covariance + dt * (
                        drift_jacobian @ covariance
                        + covariance @ drift_jacobian.T
                        + process_covariance
                    )
            ...
            sigma = observation_covariance[index].to(covariance)
            covariance = (
                jacobian_h.to(covariance) @ covariance @ jacobian_h.to(covariance).T
                + jacobian_z.to(covariance) @ sigma @ jacobian_z.to(covariance).T
            )
            covariance = stabilize_covariance(covariance, eigenvalue_floor=covariance_floor)
