...
\label{alg:framework}
\begin{algorithmic}[1]
\REQUIRE Cine-MRI sequence $\{I_k, t_k\}_{k=0}^{K}$, frozen CineMA encoder $\mathcal{E}$, frozen score network $s_{\varphi}$, trained networks $\bm{f}_\theta, \bm{G}_\theta, v_\omega, o_\chi, c_\psi$
...
\FOR{$k = 1$ to $K$}
    ...
    \STATE \hspace{\algorithmicindent}$\bm{f}_\theta, \bm{G}_\theta, \bar{\rho}_{k-1}\bm{Q}_0)$
