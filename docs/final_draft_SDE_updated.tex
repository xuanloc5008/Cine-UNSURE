...
\begin{abstract}
Quantifying uncertainty in cardiac deformation tracking is crucial for clinically reliable cardiac function assessment, yet most learning-based pipelines provide only point estimates of deformation fields and downstream metrics such as ejection fraction (EF), wall motion, and myocardial strain. We propose an uncertainty-aware neural differential registration framework for cine cardiac MRI. The model learns a continuous-time deformation trajectory from a reference cardiac frame to all observed frames using a Neural SDE-RNN, while analytically propagating observation uncertainty estimated by C-UNSURE through a frozen CineMA encoder, a covariance-aware recurrent update, and a deformation decoder. The deformation field is trained with a NODEO-style registration objective: warped-image similarity, fold prevention, motion magnitude control, and spatial/temporal smoothness regularization. CineMA serves as a frozen latent observation model, and C-UNSURE supplies frame-specific image-noise covariance that is propagated into latent observation covariance without assuming a known scalar noise level. The resulting hidden-state covariance is transformed into deformation-field covariance and subsequently into uncertainty bands for clinical metrics. Thus, the proposed framework targets not only accurate cardiac motion estimation but also clinically meaningful uncertainty propagation from image noise to deformation uncertainty and finally to biomarker confidence intervals. We validate the framework on cardiac cine-MRI sequences by assessing registration accuracy, deformation regularity, calibration of deformation uncertainty, and coverage of clinical metric prediction bands.
\end{abstract}
...
\begin{enumerate}
    ...
    \item \textbf{CineMA and C-UNSURE as an observation-uncertainty model.} We employ the pretrained CineMA encoder as a frozen latent observation model and use C-UNSURE to estimate frame-specific image noise without assuming a known scalar noise level. This image-space covariance is propagated through the frozen encoder to obtain latent observation covariance.
    ...
    \item \textbf{Self-calibrating prediction bands.} We introduce an innovation-based adaptive mechanism that, at each observed cardiac frame, compares the predicted uncertainty with the actual observation residual and adjusts the process noise covariance accordingly. This closed-loop correction ensures that the prediction bands remain well-calibrated throughout the cardiac cycle without external calibration sets.
...
The remainder of this paper is organized as follows. Section~\ref{sec:related} reviews related work. Section~\ref{sec:method} presents the proposed framework. Section~\ref{sec:clinical} derives the uncertainty propagation to clinical metrics. Section~\ref{sec:calibration} describes the self-calibration mechanism. Section~\ref{sec:experiments} presents experimental results. Section~\ref{sec:conclusion} concludes the paper.
...
\begin{enumerate}
    ...
    \item Provide prediction bands $[\mathcal{M}^{-}, \mathcal{M}^{+}]$ that are analytically derived and self-calibrating.
\end{enumerate}
...
\begin{equation}
    ...
    \label{eq:img_noise}
\end{equation}
where $\ast$ denotes spatial convolution and both the magnitude and the correlation structure of $\sigma$ are unknown. C-UNSURE estimates this structure by relaxing the zero-divergence constraint of cross-validation to a \emph{zero expected divergence} constraint over a bounded correlation support $\pm r$ taps, solving
\begin{equation}
    \min_{f}\ \max_{\eta}\
    \E_{I}\!\left[
        \|f(I) - I\|^2
        + \sum_{j=-r}^{r} \eta_j \sum_{i}
          \frac{\partial f_i}{\partial I_{(i+j)\bmod n}}(I)
    \right].
    \label{eq:cunsure}
\end{equation}
The optimal multipliers admit the closed form $\hat{\eta}_k = \mathcal{F}^{-1}\!\big(1 / \mathcal{F} h_k\big)$, where $h_k$ is the $\pm r$-tap autocorrelation of the score $\nabla \log p_I$ and $\mathcal{F}$ is the discrete Fourier transform. The estimated image-space noise covariance is the corresponding circulant matrix,
\begin{equation}
    \bm{\Sigma}_k^{\text{img}} = \operatorname{circ}(\hat{\eta}_k).
    \label{eq:img_cov}
\end{equation}
Crucially, only an \emph{upper bound} on the correlation radius $r$ is required; the per-direction variances need not be specified. Over-specifying $r$ incurs only a mild penalty, whereas under-specifying it degrades performance, so $r$ may be chosen generously. We estimate $\hat{\eta}_k$ per frame, allowing $\bm{\Sigma}_k^{\text{img}}$ to track noise that varies across slices and subjects.
...
\begin{align}
    ...
    \label{eq:mc_jvp}
\end{align}
where $\tau$ is a small constant and $\bm{\kappa}_k$ is the square-root kernel of $\bm{\Sigma}_k^{\text{img}}$, obtained from $\hat{\eta}_k$ by an elementwise square root in the Fourier domain so that correlated probes can be drawn at the cost of a single FFT. Each probe requires only one forward pass through the frozen encoder. For large $d$, $\hat{\bm{\Sigma}}_k$ may be retained in full, low-rank, or block-diagonal form, consistent with the block-diagonal approximation already adopted for $\bm{R}_k$ in Eq.~\eqref{eq:output_cov}.

\paragraph{Relation to self-calibration.}
The data-driven $\bm{\Sigma}_k$ and the innovation-based mechanism of Section~\ref{sec:calibration} address complementary sources of uncertainty. The adaptive scaling $\bm{Q}_k = \bar{\rho}_k \bm{Q}_0$~\eqref{eq:adaptive_Q} adjusts the \emph{process} (epistemic) noise by a single scalar and therefore cannot alter the \emph{structure} of the observation covariance. C-UNSURE supplies precisely that structure---an anisotropic, spatially correlated estimate of the \emph{observation} (aleatoric) noise---grounded in the noise physics rather than in a reconstruction-error heuristic. Together they close the loop on both noise terms in Eq.~\eqref{eq:adaptive_cov_prop}, and the estimate $\hat{\bm{\Sigma}}_k$ replaces $\bm{\Sigma}_k$ in Eqs.~\eqref{eq:cov_update} and~\eqref{eq:innov_cov} without any other modification to the pipeline.

\paragraph{Remarks.}
Three points warrant attention. First, the C-UNSURE multiplier is a \emph{conservative} estimate of the noise level ($\hat{\eta} \ge \sigma^2$), so the resulting bands are, if anything, slightly wider---a desirable property for clinical uncertainty quantification. Second, the validity of the encoder linearisation~\eqref{eq:enc_lin} should be checked at the operating noise level by comparing $\|\bm{J}_{\mathcal{E}}\bm{n}\|$ against second-order terms, mirroring the linearisation caveat discussed in Section~\ref{sec:discussion}. Third, Eq.~\eqref{eq:img_cov} assumes stationary (circulant) noise across the image; where g-factor variation makes the noise spatially non-stationary, $\bm{\Sigma}_k$ can be estimated block-wise per region, or with the diagonal C-UNSURE parameterisation $\bm{\Sigma}_\eta = \diag(\bm{\eta})$, which permits a distinct unknown noise level per location while remaining more expressive than cross-validation.
...
\end{equation}
In the calibrated version, $\bm{Q}_0$ is replaced by $\bar{\rho}_k\bm{Q}_0$, where $\bar{\rho}_k$ is the running innovation calibration factor.
...
The deformation field is learned without ground-truth displacement labels using the same principle as NODEO: optimize a differentiable registration objective between the warped reference frame and the observed target frame, while regularizing the transformation. CineMA and C-UNSURE are frozen and are used to provide latent observations and observation uncertainty for the CVRNN/covariance update; they do not replace the image registration objective. The trainable components are the SDE drift $\bm{f}_\theta$, diffusion $\bm{G}_\theta$, CVRNN update $v_\omega$, latent observation decoder $o_\chi$ used for innovation monitoring, and deformation decoder $c_\psi$.
...
\begin{align}
    ...
    \label{eq:l_total}
\end{align}
The latent CineMA observation and C-UNSURE covariance are used in the CVRNN update and the innovation-based calibration in Section~\ref{sec:calibration}; they are not introduced as additional primary registration losses.

\subsection{Training Procedure}
\label{sec:training_procedure}

Training proceeds in four stages. First, each cine frame is encoded by the frozen CineMA encoder, C-UNSURE estimates the image-noise kernel $\hat{\eta}_k$, and Eq.~\eqref{eq:mc_jvp} produces $\bm{\Sigma}_k$. Second, frames from the same patient and view are ordered by cardiac time, forming samples $\{I_k,\tilde{\bm{z}}_k,\bm{\Sigma}_k,t_k\}_{k=0}^{K}$. Third, the model alternates SDE propagation and CVRNN updates, decodes $\widehat{\bm{\phi}}_k$, warps the reference frame, and optimizes the NODEO-style loss in Eq.~\eqref{eq:l_total}. Fourth, validation monitors image similarity, Dice/MCD on annotated frames when labels are available, folding rate $\det\nabla\phi_k\leq0$, and the normalized innovation ratio in Section~\ref{sec:calibration}. The checkpoint is selected by registration quality subject to stable deformation regularity and calibrated innovations.
...
%==============================================================
\section{Self-Calibrating Prediction Bands}
\label{sec:calibration}
%==============================================================

A known limitation of linearization-based uncertainty propagation is that the covariance estimates may be systematically biased due to nonlinear effects. To address this, we introduce an innovation-based adaptive calibration mechanism that adjusts the process noise online.

\subsection{Innovation Monitoring}

At each observed frame $t_k$, we compute the innovation (prediction residual):
\begin{equation}
    \bm{e}_k = \tilde{\bm{z}}_k - o_\chi(\hat{\bm{h}}'_k,t_k),
    \label{eq:innovation}
\end{equation}
and the predicted innovation covariance:
\begin{equation}
    \bm{S}_k = \bm{\Delta}_h o_\chi \, \bm{P}'_k \, (\bm{\Delta}_h o_\chi)^\top + \bm{\Sigma}_k.
    \label{eq:innov_cov}
\end{equation}

In a well-calibrated system, the normalized innovation squared $\bm{e}_k^\top \bm{S}_k^{-1} \bm{e}_k$ should follow a $\chi^2$ distribution with degrees of freedom equal to the observation dimension. We monitor the ratio:
\begin{equation}
    \rho_k = \frac{1}{d} \bm{e}_k^\top \bm{S}_k^{-1} \bm{e}_k,
    \label{eq:nis}
\end{equation}
where $d$ is the observation dimension. If the system is well-calibrated, $\E[\rho_k] = 1$.

\subsection{Adaptive Process Noise Scaling}

We maintain a running estimate of the calibration ratio using an exponential moving average:
\begin{equation}
    \bar{\rho}_k = (1 - \lambda)\bar{\rho}_{k-1} + \lambda \rho_k,
    \label{eq:ema}
\end{equation}
where $\lambda \in (0,1)$ is the forgetting factor. The process noise covariance is then scaled:
\begin{equation}
    \bm{Q}_k = \bar{\rho}_k \cdot \bm{Q}_0,
    \label{eq:adaptive_Q}
\end{equation}
where $\bm{Q}_0$ is the nominal (learned) diffusion covariance.

This mechanism has a clear interpretation:
\begin{itemize}
    \item $\bar{\rho}_k > 1$: The innovations are larger than predicted, indicating \emph{overconfident} covariance estimates. The process noise is scaled up, widening the prediction bands.
    \item $\bar{\rho}_k < 1$: The innovations are smaller than predicted, indicating \emph{underconfident} covariance estimates. The process noise is scaled down, tightening the bands.
    \item $\bar{\rho}_k \approx 1$: The system is well-calibrated.
\end{itemize}

The overall SDE covariance propagation \eqref{eq:cov_prop} becomes:
\begin{equation}
    \frac{d\bm{P}}{dt} = \bm{P}\bm{F}_h^\top + \bm{F}_h\bm{P} + \bm{G}_h\,(\bar{\rho}_k \bm{Q}_0)\,\bm{G}_h^\top,
    \label{eq:adaptive_cov_prop}
\end{equation}
closing the loop between observation residuals and prediction band width.
...
\label{alg:framework}
\begin{algorithmic}[1]
\REQUIRE Cine-MRI sequence $\{I_k, t_k\}_{k=0}^{K}$, frozen CineMA encoder $\mathcal{E}$, trained networks $\bm{f}_\theta, \bm{G}_\theta, v_\omega, o_\chi, c_\psi$
\STATE \textbf{Encode:} $\bm{z}_k \gets \mathcal{E}(I_k)$ for all $k$
\STATE \textbf{Observation covariance:} estimate $\hat{\eta}_k$ with C-UNSURE and compute $\bm{\Sigma}_k$ by Eq.~\eqref{eq:mc_jvp}
\STATE Initialize $\hat{\bm{h}}_0, \bm{P}_0 \gets \bm{0}$; $\bar{\rho}_0 \gets 1$
\FOR{$k = 1$ to $K$}
    \STATE \textbf{SDE Propagation:}
    \STATE $\hat{\bm{h}}'_k, \bm{P}'_k \gets \text{SDESolve}(\hat{\bm{h}}_{k-1}, \bm{P}_{k-1}, (t_{k-1}, t_k), \bm{f}_\theta, \bm{G}_\theta, \bar{\rho}_{k-1}\bm{Q}_0)$
    \IF{frame $I_k$ is observed}
        \STATE \textbf{Innovation:}
        \STATE $\bm{e}_k \gets \tilde{\bm{z}}_k - o_\chi(\hat{\bm{h}}'_k,t_k)$
        \STATE $\bm{S}_k \gets \bm{\Delta}_h o_\chi \, \bm{P}'_k \, (\bm{\Delta}_h o_\chi)^\top + \bm{\Sigma}_k$
        \STATE $\rho_k \gets \frac{1}{d}\bm{e}_k^\top \bm{S}_k^{-1}\bm{e}_k$
        \STATE $\bar{\rho}_k \gets (1-\lambda)\bar{\rho}_{k-1} + \lambda\rho_k$
...
\ENDFOR
\RETURN $\{\hat{\mathcal{M}}_k \pm z_{\alpha/2}\sigma_{\mathcal{M},k}\}_{k=0}^{K}$
\end{algorithmic}
\end{algorithm}
...
We evaluate on the ACDC dataset \cite{bernard2018deep}, comprising 100 patients with cine-MRI sequences (12--35 frames per cycle). Expert segmentation labels for the left ventricle (LV), right ventricle (RV), and myocardium (MYO) are provided at end-diastole (ED) and end-systole (ES). We use a 70/10/20 train/validation/test split.
...
Our analytical bands are patient-specific, time-varying, and spatially varying. They adapt to the local deformation complexity and image quality without requiring any calibration data beyond the training set. The self-calibration mechanism (Section~\ref{sec:calibration}) provides an \emph{online} analog of conformal calibration, adjusting the bands at each observed frame.
